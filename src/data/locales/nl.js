export const nl = {
    "ui": {
        "start": "LATEN WE GAAN!",
        "available_sizes": "BESCHIKBARE MATEN",
        "meats_label": "VLESSELECTIE",
        "meats_max": "Maximaal toegestaan",
        "base_label": "KIES BASIS",
        "sauces_label": "SAUZEN",
        "menu_upgrade_title": "COMPLEET MET MENU?",
        "menu_upgrade_desc": "Inclusief een portie friet en een drankje naar keuze (+€2)",
        "close_total": "AFRONDEN - TOTAAL",
        "sold_out": "UITVERKOCHT",
        "dev_by": "Ontwikkeld door AyoubDev",
        "empty_category": "Geen producten beschikbaar in deze sectie.",
        "preparing": "In voorbereiding...",
        "kitchen_title": "KEUKENBEHEER",
        "update_every": "Synchronisatie elke",
        "clear_kitchen": "Bestellingen wissen",
        "cancel": "Annuleren",
        "ready": "Klaar!",
        "delivered": "Geleverd",
        "cook": "Koken",
        "ticket": "Bestelticket",
        "go_to_sauces": "BEKIJK BESCHIKBARE SAUZEN",
        "go_to_sauces_desc": "Kies je favoriete sauzen",
        "allergens_label": "Allergenen",
        "allergen_1": "Gluten",
        "allergen_2": "Schaaldieren",
        "allergen_3": "Eieren",
        "allergen_4": "Vis",
        "allergen_5": "Pinda's",
        "allergen_6": "Soja",
        "allergen_7": "Zuivel",
        "allergen_8": "Noten",
        "allergen_9": "Selderij",
        "allergen_10": "Mosterd",
        "allergen_11": "Sesam",
        "allergen_12": "Sulfieten",
        "allergen_13": "Weekdieren",
        "allergen_14": "Lupine",
        "allergen_15": "Pittig",
        "allergen_16": "Vlees",
        "new": "NIEUW!",
        "supplements_label": "SUPPLEMENTEN EN EXTRA'S",
        "durum_no_lettuce_warning": "Als er geen sla wordt toegevoegd, wordt een 'Solo Carne' toeslag berekend.",
        "extra_meat": "Extra vlees / Alleen vlees",
        "no_lettuce": "Geen sla (Meer vlees)",
        "goat_cheese": "Geitenkaas",
        "feta_cheese": "Feta kaas",
        "fried_egg": "Gebakken ei",
        "bacon": "Spek",
        "fries_inside": "Patat erin",
        "jalapenos": "Jalapeños",
        "extra_cheddar": "Extra Cheddar Kaas",
        "nav_home": "STARTPAGINA",
        "nav_location": "LOCATIE",
        "nav_hours": "OPENINGSTIJDEN",
        "nav_delivery": "BEZORGING",
        "nav_contact": "CONTACT",
        "view_menu_btn": "MENUKAART BEKIJKEN",
        "hero_prev": "Vorige",
        "hero_next": "Volgende",
        "hero_subtitle": "Kebab, Pizza's, Burgers en meer. De beste smaak van Blanes.",
        "featured_title": "ONZE FAVORIETEN",
        "featured_subtitle": "ONTDEK HET BESTE",
        "reviews_title": "WAT ZE OVER ONS ZEGGEN",
        "google_maps_text": "op Google Maps",
        "location_title": "LOCATIE",
        "view_map_btn": "KAART BEKIJKEN",
        "hours_title": "OPENINGSTIJDEN",
        "hours_loading": "Laden",
        "closed_today": "VANDAAG GESLOTEN",
        "open_now": "NU OPEN",
        "opens_at": "OPENT OM",
        "closed": "GESLOTEN",
        "day_0": "Maandag",
        "day_1": "Dinsdag",
        "day_2": "Woensdag",
        "day_3": "Donderdag",
        "day_4": "Vrijdag",
        "day_5": "Zaterdag",
        "day_6": "Zondag",
        "delivery_title": "BESTEL BEZORGING",
        "order_glovo": "BESTEL OP GLOVO",
        "glovo_note": "Je favoriete eten thuis met Glovo",
        "order_ubereats": "BESTEL OP UBER EATS",
        "ubereats_note": "Geniet van ons menu op Uber Eats",
        "local_order_label": "BEL EN BESTEL",
        "delivery_fee_note": "Gratis verzending in Blanes, 1€/km daarbuiten",
        "footer_dev_by": "ONTWORPEN & ONTWIKKELD DOOR"
    },
    "direction": "ltr",
    "categories": {
        "Tapas y Entrantes": "Tapas & Voorgerechten",
        "Dürüm": "Dürüm",
        "Pita": "Pita",
        "Combo Box": "Combo Box",
        "Tacos 303": "Tacos 303",
        "Hamburguesas": "Burgers",
        "Pizzas": "Pizza's",
        "Platos": "Schotels",
        "Postres": "Desserts",
        "Bebidas Frías": "Bebidas Frías",
        "Batidos": "Batidos",
        "Bebidas Calientes": "Warme Dranken",
        "Menús y Combos": "Menu's & Combo's",
        "Salsas": "Sauzen",
        "Suplementos": "Suplementos"
    },
    "products": {
        "Mozzarella Sticks": {
            "name": "Mozzarellasticks",
            "description": "Gepaneerde and knapperig gefrituurde mozzarellasticks. (6 stuks)"
        },
        "Alitas de Pollo": {
            "name": "Kippenvleugels",
            "description": "Gemarineerde hele kippenvleugels, op traditionele wijze gefrituurd. (6 stuks)"
        },
        "Croquetas de Pollo": {
            "name": "Kipkroketten",
            "description": "Romige kroketten gevuld met zachte bechamelsaus en stukjes kip. (6 stuks)"
        },
        "Croquetas de Pescado Maxi": {
            "name": "Maxi Viskroketten",
            "description": "Grote viskroketten met een knapperig paneerlaagje. (4 stuks)"
        },
        "Nugguets de Pollo": {
            "name": "Kipnuggets",
            "description": "Gepaneerde en gefrituurde stukjes kipfilet. (6 stuks)"
        },
        "Tenders de Pollo": {
            "name": "Kiptenders",
            "description": "Gemarineerde kiphaasjes of -reepjes met een extra knapperig paneerlaagje. (6 stuks)"
        },
        "Ensalada Mixta": {
            "name": "Gemengde Salade",
            "description": "Verfrissende basis van sla met tomaat, ui en een selectie seizoensgroenten."
        },
        "Ensalada Mix Queso Cabra": {
            "name": "Ensalada Mix Queso Cabra",
            "description": "Gemengde sla met geitenkaas medaillon."
        },
        "Nuestras Patatas Bravas": {
            "name": "Onze Patatas Bravas",
            "description": "Gefrituurde aardappelblokjes geserveerd met een klassieke, licht pittige bravasaus."
        },
        "Nuestras Patatas Fritas": {
            "name": "Onze Frietjes",
            "description": "Portie frietjes, perfect gezouten."
        },
        "Huevos rotos con Dürüm": {
            "name": "Gebakken eieren met Dürüm",
            "description": "Bedje van frietjes met daarop gebroken gebakken eieren en gekruid dürüm-kebabvlees."
        },
        "Dürüm de Pollo": {
            "name": "Kip Dürüm",
            "description": "Grote tarwetortilla gevuld met dunne reepjes gegrilde kip, groenten, salade en huissauzen."
        },
        "Dürüm de Ternera": {
            "name": "Rundvlees Dürüm",
            "description": "Tarwetortilla gevuld met gegrild rundvlees van het klassieke spit, verse groenten, salade en sauzen."
        },
        "Dürüm Mix": {
            "name": "Gemengde Dürüm",
            "description": "Tarwetortilla gevuld met een royale mix van gegrilde kip en rundvlees, groenten, salade en sauzen."
        },
        "Dürüm de Falafel": {
            "name": "Falafel Dürüm",
            "description": "Vegetarische tarwetortilla gevuld met knapperige falafelballetjes, verse salade, groenten en sauzen."
        },
        "Pita de Pollo": {
            "name": "Kip Pita",
            "description": "Broodje in luchtig Turks brood gevuld met malse gegrilde kip, verse groenten, gemengde salade en huissauzen."
        },
        "Pita de Ternera": {
            "name": "Rundvlees Pita",
            "description": "Broodje in Turks brood gevuld met dunne reepjes gegrild rundvlees, verse groenten en sauzen."
        },
        "Pita Mix": {
            "name": "Gemengde Pita",
            "description": "Broodje in Turks brood gevuld met een combinatie van gegrilde kip en rundvlees, groenten en sauzen."
        },
        "Pita Falafel": {
            "name": "Falafel Pita",
            "description": "Vegetarisch broodje in Turks brood met gefrituurde kichererwtenballetjes (falafel), groenten, salade en sauzen."
        },
        "Combo Box Nugguets": {
            "name": "Combo Box Nugguets",
            "description": "Patatas, Nugguets, Salsa Queso"
        },
        "Combo Box Tenders": {
            "name": "Combo Box Tenders",
            "description": "Patatas, Tenders, Salsa Queso"
        },
        "Combo Box Mix": {
            "name": "Combo Box Mix",
            "description": "Patatas, Nugguets, Tenders, Salsa Queso"
        },
        "Taco de Pollo": {
            "name": "Kip Taco (Frans)",
            "description": "Op Franse wijze: grote tarwetortilla gegrild en gevuld met malse gemarineerde kip, Algerijnse of Andalusische saus, gesmolten mozzarella, kaassaus en frietjes binnenin."
        },
        "Taco de Ternera": {
            "name": "Rundvlees Taco (Frans)",
            "description": "Op Franse wijze: gevulde tortilla met gekruid rundergehakt, frietjes binnenin, mozzarella, romige kaassaus en keuze uit Algerijnse of Andalusische saus."
        },
        "Taco Mix": {
            "name": "Gemengde Taco (Frans)",
            "description": "Op Franse wijze: gevulde tortilla met een mix van vleessoorten, frietjes, rijke kaassaus, gesmolten mozzarella en Algerijnse of Andalusische saus."
        },
        "Taco de Nugguets": {
            "name": "Nuggets Taco (Frans)",
            "description": "Originele Franse taco gevuld met knapperige kipnuggets, frietjes, mozzarella, kaassaus en Algerijnse of Andalusische saus."
        },
        "Taco de Tenders": {
            "name": "Tenders Taco (Frans)",
            "description": "Stevige Franse taco gevuld met gepaneerde kiphaasjes (tenders), frietjes, veel gesmolten mozzarella, kaassaus en Algerijnse of Andalusische saus."
        },
        "Hamburguesa Americana": {
            "name": "American Burger",
            "description": "Burger op rustiek brood met 200g rundvlees, augurk, gesmolten cheddar, gekaramelliseerde ui en speciale American saus. Inclusief frietjes."
        },
        "Hamburguesa Clássica": {
            "name": "Classic Burger",
            "description": "Burger op rustiek brood with 200g rundvlees, sla, plakjes tomaat, gesmolten Edammer kaas en een sappig gebakken ei. Inclusief frietjes."
        },
        "Hamburguesa Queso de Cabra": {
            "name": "Geitenkaas Burger",
            "description": "Gourmet burger op rustiek brood met 200g rundvlees, verse rucola, gegrilde geitenkaas, gekaramelliseerde ui en honing-mosterdsaus. Inclusief frietjes."
        },
        "Hamburguesa Chicken Crispy": {
            "name": "Chicken Crispy Burger",
            "description": "Burger op een zacht briochebroodje gevuld met 200g knapperig gepaneerde kip, rucola, gesmolten cheddar, ranchsaus en augurk. Inclusief frietjes."
        },
        "Pizza Margarita": {
            "name": "Pizza Margherita",
            "description": "Klassiek Italiaans pizzadeeg belegd met tomatensaus, gesmolten mozzarella en verse basilicum."
        },
        "Pizza Italia": {
            "name": "Pizza Italia",
            "description": "Pizzadeeg met tomatensaus, verse blokjes tomaat, rucola en mozzarella."
        },
        "Pizza Kebab": {
            "name": "Pizza Kebab",
            "description": "Italiaans-Turkse fusie: pizzadeeg met tomaat en mozzarella, belegd met kebabvlees naar keuze, olijven en een vleugje witte kebabsaus."
        },
        "Pizza de Pollo": {
            "name": "Pizza Kip",
            "description": "Pizzadeeg met tomaat en mozzarella, belegd met lekkere gekaramelliseerde ui, paprika en malse stukjes kip."
        },
        "Pizza Barbacoa": {
            "name": "Pizza Barbecue",
            "description": "Pizzadeeg belegd met tomatensaus, mozzarella, rundergehakt, gerookte spekreepjes, rode paprika en een scheut barbecuesaus."
        },
        "Pizza Tonno": {
            "name": "Pizza Tonno",
            "description": "Pizza met tomatensaus, mozzarella, stukjes tonijn, ui, plakjes hardgekookt ei en zwarte olijven."
        },
        "Pizza Vegetal": {
            "name": "Vegetarische Pizza",
            "description": "Pizzadeeg belegd met tomatensaus en veel groenten: spinazie, plakjes courgette en aubergine, paprika en olijven."
        },
        "Pizza 4 Quesos": {
            "name": "4 Kazen Pizza",
            "description": "Voor kaasliefhebbers: tomatensaus en een smeltende mix van gorgonzola, brie, geitenkaas en klassieke mozzarella."
        },
        "Pizza Infierno": {
            "name": "Pizza Inferno (Pittig)",
            "description": "Pittig pizzadeeg belegd met tomatensaus, plakjes chorizo, pittige saus of kruiden en mozzarella."
        },
        "Pizza Mediterránea": {
            "name": "Mediterrane Pizza",
            "description": "Pizzadeeg met tomatensaus, garnalen, ansjovis, een licht pittig accent en mozzarella."
        },
        "Pizza Hawaiana": {
            "name": "Pizza Hawaï",
            "description": "Tropisch pizzadeeg met tomatensaus, kalkoenham, zoete stukjes ananas, maïs en een scheutje room."
        },
        "Pizza a tu Gusto": {
            "name": "Pizza a tu Gusto",
            "description": "Medium pizza naar wens belegd."
        },
        "Plato Kebab de Pollo": {
            "name": "Kebabschotel Kip",
            "description": "Royale portie op een bord (zonder brood) met reepjes gegrilde kip, geserveerd met gemengde salade en frietjes."
        },
        "Plato Kebab de Ternera": {
            "name": "Kebabschotel Rund",
            "description": "Bord met smakelijke reepjes gegrild rundvlees, geserveerd met verse gemengde salade en frietjes."
        },
        "Plato Kebab Mix": {
            "name": "Gemengde Kebabschotel",
            "description": "Combischotel met het beste van beide werelden: een mix van gegrilde kip en rundvlees, geserveerd met salade en frietjes."
        },
        "Plato Falafel": {
            "name": "Falafelschotel",
            "description": "Vegetarisch alternatief op een bord: meerdere falafelballetjes, geserveerd met bijsalade en frietjes."
        },
        "Tarta de Queso": {
            "name": "Cheesecake",
            "description": "Een punt zachte en romige cheesecake als zoete afsluiter."
        },
        "Agua Mineral 0,5L": {
            "name": "Mineraalwater 0,5L",
            "description": "Flesje natuurlijk mineraalwater zonder koolzuur. 0,5L."
        },
        "Agua Mineral 1,5L": {
            "name": "Mineraalwater 1,5L",
            "description": "Fles natuurlijk mineraalwater zonder koolzuur. Grootverpakking 1,5L."
        },
        "Zumo de Melocotón": {
            "name": "Perziksap",
            "description": "Flesje perziksap of -nectar."
        },
        "Cacaolat": {
            "name": "Cacaolat",
            "description": "Traditionele chocolademelk, koud geserveerd."
        },
        "Coca-Cola": {
            "name": "Coca-Cola",
            "description": "Koude frisdrank (Coca-Cola)."
        },
        "Coca-Cola Zero": {
            "name": "Coca-Cola Zero",
            "description": "Koude frisdrank (Coca-Cola Zero)."
        },
        "Fanta Naranja": {
            "name": "Fanta Naranja",
            "description": "Sinaasappel-Fanta (koud)."
        },
        "Fanta Limón": {
            "name": "Fanta Limón",
            "description": "Citroen-Fanta (koud)."
        },
        "Aquarius Naranja": {
            "name": "Aquarius Naranja",
            "description": "Isotone sinaasappeldrank (koud)."
        },
        "Aquarius Limón": {
            "name": "Aquarius Limón",
            "description": "Isotone citroendrank (koud)."
        },
        "Nestea Limón": {
            "name": "Nestea Limón",
            "description": "Citroen-ijsthee (koud)."
        },
        "Nestea Maracuyá": {
            "name": "Nestea Maracuja",
            "description": "Koude frisdrank (Nestea Maracuja)."
        },
        "Trina Naranja": {
            "name": "Trina Naranja",
            "description": "Trina sinaasappeldrank (plat)."
        },
        "Pepsi": {
            "name": "Pepsi",
            "description": "Pepsi-Cola (koud)."
        },
        "Sprite": {
            "name": "Sprite",
            "description": "Koude frisdrank (Sprite)."
        },
        "7UP": {
            "name": "7UP",
            "description": "Individueel koud citroen-limoen frisdrank."
        },
        "Hawai": {
            "name": "Hawai",
            "description": "Koude frisdrank (Hawai)."
        },
        "Poms": {
            "name": "Poms",
            "description": "Koude frisdrank (Poms)."
        },
        "Red Bull": {
            "name": "Red Bull",
            "description": "Koude frisdrank (Red Bull)."
        },
        "Monster": {
            "name": "Monster",
            "description": "Individueel koud energiedrankje."
        },
        "Vichy Catalán": {
            "name": "Vichy Catalán",
            "description": "Koude frisdrank (Vichy Catalán)."
        },
        "Schweppes": {
            "name": "Schweppes",
            "description": "Koude frisdrank (Schweppes)."
        },
        "Mango Lassi": {
            "name": "Mango Lassi",
            "description": "Verfrissende traditionele Indiase drank op basis van yoghurt en mangopulp."
        },
        "Café con leche": {
            "name": "Koffie met melk",
            "description": "Een kop koffie verkeerd (lungo met gestoomde melk)."
        },
        "Cortado": {
            "name": "Cortado",
            "description": "Kleine espresso met een wolkje melk."
        },
        "Café solo": {
            "name": "Koffie zwart",
            "description": "Een pure zwarte espresso."
        },
        "Infusiones": {
            "name": "Thee / Kruidenthee",
            "description": "Heet water met een theezakje naar keuze."
        },
        "Menú Dürüm": {
            "name": "Dürüm-menu",
            "description": "Compleet menu voor één persoon inclusief een dürüm naar keuze, frietjes en een frisdrankje."
        },
        "Menú Pita": {
            "name": "Pita-menu",
            "description": "Compleet menu voor één persoon inclusief een broodje pita naar keuze, frietjes en een frisdrankje."
        },
        "Menú Taco": {
            "name": "Taco-menu",
            "description": "Compleet menu voor één persoon inclusief een Franse Taco naar keuze, frietjes en een frisdrankje."
        },
        "Menú Hamburguesa": {
            "name": "Burger-menu",
            "description": "Compleet menu voor één persoon inclusief een burger naar keuze, frietjes en een frisdrankje."
        },
        "Menú Pizza": {
            "name": "Pizza-menu",
            "description": "Menu inclusief een 32cm pizza naar keuze en een frisdrankje."
        },
        "Combo 2 Dürüms": {
            "name": "2 Dürüms-combo",
            "description": "Deelmenu met twee dürüms naar keuze, twee porties friet en twee frisdrankjes."
        },
        "Combo 2 Pitas": {
            "name": "2 Pitas-combo",
            "description": "Deelmenu met twee broodjes pita naar keuze, twee porties friet en twee frisdrankjes."
        },
        "Combo 2 Pizzas Medianas": {
            "name": "Combo 2 Pizzas Medianas",
            "description": "Twee medium pizza's (32cm) naar keuze."
        },
        "Combo Family": {
            "name": "Familie-combo",
            "description": "Het grootste pakket voor groepen of families. Bevat: 1 Dürüm, 1 Pita, 1 Burger naar keuze, 1 pizza 32cm, 3 porties friet en een grote fles frisdrank."
        },
        "Menú Infantil": {
            "name": "Menú Infantil",
            "description": "Kindermenu met krokante kipnuggets, friet en sap."
        },
        "Salsa Algerian": {
            "name": "Algerijnse saus",
            "description": "Extra bakje Algerijnse saus."
        },
        "Salsa Andaluza": {
            "name": "Andalusische saus",
            "description": "Extra bakje Andalusische saus."
        },
        "Salsa Barbacoa": {
            "name": "Barbecuesaus",
            "description": "Extra bakje barbecuesaus."
        },
        "Salsa Biggie": {
            "name": "Biggie saus",
            "description": "Extra bakje Biggie saus."
        },
        "Salsa Brazil": {
            "name": "Brazil saus",
            "description": "Extra bakje Brazil saus."
        },
        "Salsa Samurai": {
            "name": "Samuraisaus",
            "description": "Extra bakje samuraisaus."
        },
        "Salsa Ketchup": {
            "name": "Ketchup",
            "description": "Extra bakje ketchup."
        },
        "Salsa Mayonesa": {
            "name": "Mayonaise",
            "description": "Extra bakje mayonaise."
        },
        "Solo Carne (Sin ensalada/patatas)": {
            "name": "Solo Carne (Sin ensalada/patatas)",
            "description": "Puur kebabvlees bord, zonder bijgerechten."
        },
        "Extra de Carne": {
            "name": "Extra de Carne",
            "description": "Extra portie kebabvlees."
        },
        "Queso Feta": {
            "name": "Queso Feta",
            "description": "Portie Griekse feta kaas."
        },
        "Queso de Cabra": {
            "name": "Queso de Cabra",
            "description": "Geitenkaas medaillon."
        },
        "Ingrediente Extra": {
            "name": "Ingrediente Extra",
            "description": "Extra ingrediënt."
        }
    }
,
    "pizza_custom": {
    "title": "MAAK JE EIGEN PIZZA",
    "bases": "Bases (Kies 1)",
    "quesos": "Kazensch (Meerdere)",
    "carnes": "Vlees (Meerdere)",
    "verduras": "Groenten (Meerdere)",
    "items": {
        "Tomate": "Tomaat",
        "Barbacoa": "BBQ",
        "Crema/Nata": "Room",
        "Mozzarella": "Mozzarella",
        "Gorgonzola": "Gorgonzola",
        "Queso de Cabra": "Geitenkaas",
        "Pollo": "Kip",
        "Ternera": "Rundvlees",
        "Bacón": "Spek",
        "Jamón York": "Ham",
        "Peperoni": "Pepperoni",
        "Atún": "Tonijn",
        "Cebolla": "Ui",
        "Pimiento": "Paprika",
        "Champiñones": "Champignons",
        "Olivas Negras": "Zwarte Olijven",
        "Piña": "Ananas"
    }
}
};